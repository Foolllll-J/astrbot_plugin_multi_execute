...
一款为 [**AstrBot**](https://github.com/AstrBotDevs/AstrBot) 设计的指令模拟器。支持模拟其他用户执行指令、自动连续执行指令、免唤醒触发指令、伪装指令回复和设置指令别名。
...
- 🔗 指令别名：为关键词或指令设置别名，支持唤醒前缀，别名后的参数自动保留传递给目标。
...
> [!TIP]
> - 开启 `所有指令免唤醒` 模式后，所有已注册的指令都无需前缀即可触发。
> - 可通过 `需唤醒指令列表` 设置黑名单，也可可通过 `免唤醒指令列表` 为非注册指令的关键词设置白名单。
...
用户可为指令或为未注册的关键词设置别名，并支持多对多映射。

- **示例**：目标关键词 `菜单`，别名 `[ "menu", "帮助" ]`
- **使用**：发送 `menu` 或 `帮助`
- **效果**：效果等同于触发 `菜单`。
